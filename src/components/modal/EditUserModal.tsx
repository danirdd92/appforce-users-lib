import { Dialog, Transition } from "@headlessui/react";
import { Fragment, PropsWithChildren, useState } from "react";
import { useAppDispatch, useAppSelector } from "../../store";
import { toggleSelectUser } from "../../store/usersSlice";
import { EditUserForm } from "./EditUserForm";
import { closeModal } from "../../store/modalSlice";

type Props = {
  open: boolean;
};

export const EditUserModal = ({ open }: Props) => {
  const dispatch = useAppDispatch();
  const selectedUser = useAppSelector((state) => state.usersSlice.selectedUser);
  const modalType = useAppSelector((state) => state.modalSlice.modalType);
  const isOpen = useAppSelector((state) => state.modalSlice.isOpen);

  const handleOnClose = () => {
    dispatch(closeModal);
    dispatch(toggleSelectUser(undefined));
  };

  return (
    <Transition appear show={modalType === "edit" && isOpen} as={Fragment}>
      <Dialog
        as="div"
        className="relative z-10"
        open={modalType === "edit" && isOpen}
        onClose={() => handleOnClose()}
      >
        <Transition.Child
          as={Fragment}
          enter="ease-out duration-300"
          enterFrom="opacity-0"
          enterTo="opacity-100"
          leave="ease-in duration-200"
          leaveFrom="opacity-100"
          leaveTo="opacity-0"
        >
          <div className="fixed inset-0 bg-black bg-opacity-25" />
        </Transition.Child>

        <div className="fixed inset-0 overflow-y-auto">
          <div className="flex min-h-full items-center justify-center p-4 text-center">
            <Transition.Child
              as={Fragment}
              enter="ease-out duration-300"
              enterFrom="opacity-0 scale-95"
              enterTo="opacity-100 scale-100"
              leave="ease-in duration-200"
              leaveFrom="opacity-100 scale-100"
              leaveTo="opacity-0 scale-95"
            >
              <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                <Dialog.Title
                  as="h3"
                  className="text-md font-medium leading-6 text-gray-900"
                >
                  Edit User
                </Dialog.Title>
                <div className="mt-2">
                  <EditUserForm user={selectedUser!} />
                </div>
              </Dialog.Panel>
            </Transition.Child>
          </div>
        </div>
      </Dialog>
    </Transition>
  );
};
